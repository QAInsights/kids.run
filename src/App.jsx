...
import { Home, MapPin, Clock, Plus, Trash2, Navigation2, Map as MapIcon, Settings, Route as RouteIcon, Car, CheckCircle2, RotateCcw, Zap, Sun, Moon, Eye, EyeOff, Calendar } from 'lucide-react';
import './index.css';
...
const PlaceInput = ({ value, onChange, placeholder, apiKey, icon, style, className }) => {
  ...
  useEffect(() => {
    ...
    if (inputRef.current._hasAutocomplete) return;
    ...
    inputRef.current._hasAutocomplete = true;
    return () => {
...
export default function App() {
  ...
  const [kids, setKids] = useState([
    { id: 1, name: "Ava", color: "var(--primary)", activities: [{ id: 101, name: "Volleyball", location: "", startTime: "16:00", endTime: "17:30" }] },
    { id: 2, name: "Ben", color: "var(--secondary)", activities: [{ id: 102, name: "Music Class", location: "", startTime: "16:45", endTime: "17:45" }] }
  ]);
  ...
  useEffect(() => {
    ...
    // Adding directions/maps libraries implicitly bundled
  ...
  const addActivity = (kidId) => {
    setKids(kids.map(k => k.id === kidId ? {
      ...k, activities: [...k.activities, { id: Date.now(), name: "New Activity", location: "", startTime: "16:00", endTime: "17:00" }]
    } : k));
  };
  ...
  const buildPlanFromDirections = (result, allStops) => {
     ...
     const plan = [];
     plan.push({ phase: 'Depart Home', location: homeAddress, time: 'Start', desc: 'Begin route', icon: <Home size={18} color="#fff" />, color: "var(--success)" });
     ...
     allStops.forEach((stop) => {
        ...
        plan.push({
          phase: 'Drive', location: '', time: '', desc: `Drive ${leg.distance.text} (${leg.duration.text})`,
          icon: <Car size={18} className="text-muted" />, isDrive: true, color: "var(--surface)"
        });

        plan.push({
          phase: isDropoff ? `Drop off ${stop.kid}` : `Pick up ${stop.kid}`,
     ...
     plan.push({
          phase: 'Drive', location: '', time: '', desc: `Return home: ${finalLeg.distance.text} (${finalLeg.duration.text})`,
          icon: <Car size={18} className="text-muted" />, isDrive: true, color: "var(--surface)"
     });
     plan.push({
        phase: 'Return Home', location: homeAddress, time: 'End', desc: 'End of trip',
  ...
  const buildHeuristicPlan = (allStops) => {
      const plan = [];
      plan.push({ 
        phase: 'Depart Home', location: homeAddress || 'Home Address', time: 'Start', desc: 'Start route', 
        icon: <Home size={18} color="#fff" />, color: "var(--success)"
      });
      ...
      allStops.forEach((stop, index) => {
        ...
        if (index < allStops.length - 1) {
          plan.push({
            phase: 'Drive', location: '', time: '', desc: 'Est. 15 mins travel (Google Maps off)',
            icon: <Car size={18} className="text-muted" />, isDrive: true, color: "var(--surface)"
          });
        }
      });
      ...
      plan.push({
        phase: 'Return Home', location: homeAddress || 'Home Address', time: 'End', desc: 'End of route',
        icon: <Home size={18} color="#fff" />, color: "var(--success)"
      });
  ...
  return (
    <div style={{ paddingBottom: '6rem' }}>
      ...
      <main className="container flex-col gap-6" style={{ display: 'flex', paddingTop: 0 }}>
                    ...
                    <div key={act.id} style={{ 
                      ...
                      <div className="flex-col gap-3" style={{ display: 'flex' }}>
                        ...
                        <PlaceInput 
                           ...
                           placeholder="Location Address" apiKey={apiKey && scriptLoaded ? apiKey : null}
        ...
        {tripPlan && (
          <section className="card animate-slide-up" style={{ marginTop: '2.5rem', padding: '2.5rem', backgroundColor: 'var(--text-main)', color: 'var(--bg-color)' }}>
            <div className="flex justify-between items-start mb-6">
              ...
              <button className="btn-ghost" onClick={() => { setTripPlan(null); setDirectionsResult(null); }} style={{ color: 'var(--bg-color)', opacity: 0.7 }}>
                <RotateCcw size={20} />
              </button>
            </div>
            ...
            {directionsResult && (
               <div id="map-canvas" style={{ width: '100%', height: '300px', borderRadius: '1.5rem', marginBottom: '2.5rem', overflow: 'hidden' }}></div>
            )}
